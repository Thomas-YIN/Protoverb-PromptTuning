...
class Template(nn.Module):
    ...
    def incorporate_text_example_with_label(self,
                                 example: InputExample,
                                 text = None,
                                 label:str = None
                                ):
    ...
    # @abstractmethod
    def wrap_one_example_with_label(self,
                         example: InputExample, label_words:str) -> List[Dict]:
        ...
        if isinstance(example, InputExample):
            ...
            # replace <mask> in the text by the label word,
            # need to do this after the above block so that loss_ids for <mask> are kept
            values[0] = self.incorporate_text_example_with_label(example, label=label_words[example.label])
